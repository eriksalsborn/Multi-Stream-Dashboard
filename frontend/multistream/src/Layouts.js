...
const saveLayoutsUrl =
  "https://hie7efmkul.execute-api.eu-north-1.amazonaws.com/prod/savelayouts";
const getLayoutsUrl =
  "https://hie7efmkul.execute-api.eu-north-1.amazonaws.com/prod/getlayouts";
...
const Layouts = () => {
  ...
  const handleDrag = (toDrag, dx, dy) => {
    ...
    toDrag.y = dy;
    setTwitchWindows(
      twitchWindows.map((a) => (a.url !== toDrag.url ? a : toDrag))
    );
  };
  ...
  const handleResize = (toResize, width, height, position) => {
    ...
    setTwitchWindows(
      twitchWindows.map((a) => (a.url !== toResize.url ? a : toResize))
    );
  };
